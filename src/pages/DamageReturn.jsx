...
const DamageReturn = () => {
  ...
  useEffect(() => {
    const fetchItemName = async () => {
      if (formData.code.trim()) {
        try {
          const res = await axios.get(`http://localhost:5000/api/stocks/get-by-code/${formData.code}`);
  ...
  const fetchDamageHistory = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/damage-return/history");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:5000/api/damage-return/add", payload);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this record?")) {
      try {
        await axios.delete(`http://localhost:5000/api/damage-return/delete/${id}`);
        fetchDamageHistory();
      } catch (err) {
