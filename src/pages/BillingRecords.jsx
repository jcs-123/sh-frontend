...
const BillingRecords = () => {
  ...
  const handleExport = () => {
    const worksheetData = bills.map(bill => ({
      ...
      Discount: bill.discount?.toFixed(2) || "0.00",  // Adding the discount amount to the export
      GrandTotal: bill.grandTotal?.toFixed(2) || "0.00",  // Adding the grand total (after discount)
      ...
      <TableContainer component={Paper} elevation={2}>
        <Table>
          <TableHead sx={{ backgroundColor: "#1976d2" }}>
            <TableRow>
              {["Date", "Receipt No.", "Buyer", "Total", "Discount", "Grand Total", "Payment", "Balance", "Actions"].map((header) => (
            ...
            {bills.length === 0 ? (
              ...
              bills.map((bill, index) => (
                <TableRow key={bill._id} sx={{ backgroundColor: index % 2 === 0 ? "#fafafa" : "#fff" }}>
                  ...
                  <TableCell>₹ {bill.discount?.toFixed(2) || "0.00"}</TableCell> {/* Discount column */}
                  <TableCell>₹ {bill.grandTotal?.toFixed(2) || "0.00"}</TableCell> {/* Grand total */}
